 ...
 *  path on the site. Only two things are left out: a page a moderator hid, and a page with no text. */
...
async function runPolicy(env: Env, q: { ns: string; slug: string; target: PolicyTarget; trigger: "write" | "report"; reason: string | null; caseSeq?: number }): Promise<void> {
  ...
  if (decision.action === "redact") {
    ...
    if (r.kind === "redacted" && !info.private) {
      await fh.record({ at: Date.now(), ns: q.ns, slug: q.slug, rev: r.rev, kind: "redact", by: r.by, bytes: 0, note: r.row === null ? "redacted" : `row ${r.row} redacted`, sealed: false });
...
async function moderationAction(ctx: Ctx, stub: DurableObjectStub<Namespace>, ns: string, slug: string, p: Map<string, string>, pageUrl: string, isPrivate: boolean): Promise<Response> {
  ...
  if (!action) return fail(400, "moderation needs one of &freeze=1 &unfreeze=1 &hide=1 &restore=1 &append_only=1|0 &redact=<rev> &redactrow=<n> &unredact=<rev> &unredactrow=<n> &resolve=<case>, or &set= / &add= for a sealed write");
