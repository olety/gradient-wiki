...
`GET /p/<ns>/<slug>?report=<reason>[&rev=N | &row=N][&note=<≤200>][&by=<name>]`, also POST (form or JSON). Reasons, fixed: `fraud` `crime` `threat` `csam` `doxx` `defamation` `copyright` `secret` `other`. Missing or unknown reason → `400 report needs ?report=<reason>: fraud crime threat csam doxx defamation copyright secret other`. No rev/row = the page's current revision. Private-read namespaces need the key; nothing else does. Receipt (text): `reported rev N <pageUrl> case <seq>` (or `reported row n rev N …`), then `notice: <base>/notice`; JSON `{ok:true, action:"reported", case, rev, row, url}`; HTML = the receipt view. `GET /p/<ns>/<slug>/report` = plain HTML form (reason radios, optional rev/row, note, by; POSTs to the page URL) with `X-Robots-Tag: noindex, nofollow`. A `report` link sits in the page view's action list after `history · edit · .json`, and each revision in the history view links `?report=other&rev=N`. Limit: 10 reports per hour per IP (new bucket `rep:<iphash>`, 429 `slow down: 10 reports per hour`), on top of the write bucket. `robots.txt` adds `Disallow: /*?report=`, `Disallow: /*&report=`, `Disallow: /*/report`. A report carrying `&note=` always opens a case. A report with no note opens one only when the claim — page, revision or row, and reason — is new. Only an earlier `report` case holds a claim; an `auto` case the classifier opened by itself never does, because a person reporting what the site already suspected is new information and must run the report row of the action matrix. When a report case already holds that claim the report is counted on it (`reports` + 1) and nothing else happens — a still-open case wins over a closed one, then the newest —: no second case, no `/log` line, no classifier call, no mail. Receipt (text): `rev N is already case <seq>, open <pageUrl>` or `… closed <ISO> <pageUrl>` (or `row n rev N …`), then `add &note= to say something new, or report another reason.`, then `notice: <base>/notice`; JSON `{ok:true, action:"counted", case, status, reports, rev, row, url}`. A crawler replaying a saved report URL therefore cannot wake the queue twice, and a notice from a rights holder or a named person always lands because it carries a note or a different reason. Opening a case logs `report` in `/log` for public namespaces with reason `rev N: <reason>` (the note is never published), and runs the classifier on that revision or row at once with the reason as a hint.
